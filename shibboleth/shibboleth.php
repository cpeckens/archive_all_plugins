...
/*
 ...
 Description: Easily externalize user authentication to a <a href="http://shibboleth.internet2.edu">Shibboleth</a> Service Provider. Modified by Cara Peckens for Johns Hopkins SSO
...
define ( 'SHIBBOLETH_PLUGIN_REVISION', preg_replace( '/\$Rev: (.+) \$/', '\\1',
	'$Rev: 282779 $') ); // this needs to be on a separate line so that svn:keywords can work its magic
...
function shibboleth_session_initiator_url($redirect = null) {
	...
	if ( function_exists('switch_to_blog') ) switch_to_blog($GLOBALS['current_site']->blog_id);
	$target = network_home_url('wp-login.php'); // Modified by CP - Changed get_home_url to network_home_url
	$redirect = get_home_url(null) . '/wp-admin/my-sites.php'; //Modified by CP - line added to redirect to My Sites page after JHED authentication
	if ( function_exists('restore_current_blog') ) restore_current_blog();
